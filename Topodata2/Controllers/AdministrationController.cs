    ...
    public class AdministrationController : Controller
    {
        ...
        public ContentResult GetContentForTable(int modelType)
        {
            ...
            switch (type)
            {
                ...
                case ActionType.Sectores:
                    result =
                        JsonConvert.SerializeObject(
                            ServiceManager.GetAllSubCategories().Select(selected => new AllSectoresModel
                            {
                                Id = selected.Id,
                                Descripcion = selected.Descripcion,
                                RegDate = selected.RegDate.ToShortDateString()
                ...
                case ActionType.OurEquipos:
                    result =
                        JsonConvert.SerializeObject(HomeManager.GetAllOurTeam().Select(selected => new AllOurTeamModel
                        {
                            Id = selected.Id,
                            Nombre = selected.Nombre,
        ...
        public ActionResult Documents(DocumentViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return RedirectToAction("Index",
                    new
                    {
                        ...
                        index = (int) ActionType.Documents
                    });
            }
            if (model.ImageUpload != null)
            {
                if (
                    !new[] {"image/gif", "image/jpeg", "image/pjpeg", "image/png"}.Contains(
                        model.ImageUpload.ContentType))
                {
                    return RedirectToAction("Index",
                        new
                        {
                            opStatus = General.OpStatusError,
                            opMessage = Messages.TieneFormatoImagen,
                            index = (int) ActionType.Documents
                        });
                }
            ...
            if (!ServiceManager.AddDocument(model))
                return RedirectToAction("Index",
                    ...
                    {
                        opStatus = General.OpStatusError,
                        opMessage = Messages.ErrorDesconocido,
                        index = (int) ActionType.Documents
                    });
        ...
        public ActionResult HomeSlideVideo(HomeSlideVideoViewModel viewModel)
        {
            if (!ModelState.IsValid)
                return RedirectToAction("Index",
                    new
                    {
                        ...
                        opMessage =
                            string.Join("; ", ModelState.Values.SelectMany(m => m.Errors.Select(n => n.ErrorMessage))),
                        index = (int) ActionType.HomeSlideVideo
                    });
            ...
            return RedirectToAction("Index",
                new
                {
                    opStatus = General.OpStatusSuccess,
                ...
                ? RedirectToAction("Index",
                    HomeManager.AddFlipboard(viewModel)
                        ? new
                        {
                            opStatus = General.OpStatusSuccess,
                            opMessage = (string) null,
                            index = (int) ActionType.Flipboard
                        }
                        : new
                        {
                            opStatus = General.OpStatusError,
        ...
        public ActionResult ImageSeason(HomeSliderImageSeasonViewModel viewModel)
        {
            if (!ModelState.IsValid)
                return RedirectToAction("Index",
                    new
                    {
                        ...
                        opMessage =
                            string.Join("; ", ModelState.Values.SelectMany(m => m.Errors.Select(n => n.ErrorMessage))),
                        index = (int) ActionType.ImageSeason
                    });
            ...
            if (!new[] {"image/jpeg"}.Contains(viewModel.ImageUpload.ContentType))
            {
                return RedirectToAction("Index",
                    ...
                    {
                        opStatus = General.OpStatusError,
                        opMessage = Messages.TieneFormatoJPG,
                        index = (int) ActionType.ImageSeason
                    });
            }
        ...
        public ActionResult OurTeam(OurTeamViewModel viewModel)
        {
            if (!ModelState.IsValid)
            {
                return RedirectToAction("Index",
                    new
                    {
                        ...
                        opMessage =
                            string.Join("; ", ModelState.Values.SelectMany(m => m.Errors.Select(n => n.ErrorMessage))),
                        index = (int) ActionType.OurEquipos
                    });
            }
            if (viewModel.ImageUpload != null)
            {
                if (
                    !new[] {"image/gif", "image/jpeg", "image/pjpeg", "image/png"}.Contains(
                        viewModel.ImageUpload.ContentType))
                {
                    return RedirectToAction("Index",
                        new
                        {
            ...
            return RedirectToAction("Index",
                HomeManager.AddOurTeam(viewModel)
                    ? new
                    {
                        opStatus = General.OpStatusSuccess,
                        opMessage = (string) null,
